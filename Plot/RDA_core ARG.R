...
#filter by core ARGs
colnames(core)[1]<-"ARGs abundance normalization aganist 16S"
core<-core[-27,]
dbpata<-dbpata[dbpata$`ARGs abundance normalization aganist 16S` %in% core$`ARGs abundance normalization aganist 16S`,]
dbpata<-dbpata[apply(dbpata, 1, function(x) !all(x==0)),]
rownames(dbpata)<-dbpata$`ARGs abundance normalization aganist 16S`
dbpata$`ARGs abundance normalization aganist 16S`<-NULL
#Genus 篩選
envata<-envata%>%
  filter(apply(envata,1,function(x) sum(x>0)>=12))
...
B.rda.data$Location<-factor(B.rda.data$Location,levels = c("Finished","Upstream","Midstream","Downstream"))
