...
ggplot(varaible_group_mean)+geom_bar(aes(x=location, y=type_mean), stat="identity",fill="#80B1D3",alpha=0.7,width = 0.8)+geom_errorbar(aes(x=location,ymin=type_mean-type_sd, ymax=type_mean+type_sd), width=.1,position=position_dodge(.9))+
  ...
  geom_line(data=tibble(x=c(2,5),y=c(0.145,0.145)),aes(x=x,y=y),inherit.aes = F,linewidth=0.8)+geom_text(data=tibble(x=3.5,y=0.150),aes(x=x,y=y,label="*"),size=5,inherit.aes = F)+geom_line(data=tibble(x=c(3,5),y=c(0.13,0.13)),aes(x=x,y=y),inherit.aes = F,linewidth=0.8)+geom_text(data=tibble(x=4,y=0.135),aes(x=x,y=y,label="*"),size=5,inherit.aes = F)+
  geom_line(data=tibble(x=c(4,5),y=c(0.115,0.115)),aes(x=x,y=y),inherit.aes = F,linewidth=0.8)+geom_text(data=tibble(x=4.5,y=0.120),aes(x=x,y=y,label="**"),size=5,inherit.aes = F)
#6.77 6.51
#進行anova
