...
library(GGally)
library(tidyverse)
data <- read.xlsx("C:/Users/USER/Desktop/Mycobacterium ARC.xlsx",sheet=1,rowNames =T)
#data transform----------------------------------------
data <-data%>%
...
lower_fn <- function(data, mapping, ...){
  p <- ggplot(data = data, mapping = mapping) + 
    geom_point(color="#704D9E",alpha=0.7,size=2) + 
...
diag_fn <- function(data, mapping, ...) {
  ggplot(data = data, 
         mapping = mapping) +
    geom_density(...,alpha=0.7,fill="#704D9E" ,color="#704D9E" )
}
