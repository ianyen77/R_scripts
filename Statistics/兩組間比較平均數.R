...
{#檢查數據是否是常態分布的,利用shapiro.test來檢驗數據是不是常態的，如果p>0.05那麼數據就是常態的
  ...
  for(i in seq(length(Data.levels))) {
    ...
    if (group.n < 50) {
      shapiro.result = shapiro.test(Data.levels[[i]]$tetracycline)
...
pairwise.t.test(data$tetracycline,data$location,p.adjust.method = "BH")

#如果兩者中有一不符合，那我們得使用wilcoxon rank sum test
pairwise.wilcox.test(data$aminoglycoside,data$location,p.adjust.method = "BH",paired=F)
